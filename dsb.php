...
//  NOTE: New schema — filter by user_id (no store_id).
// ============================================================
...
try {
    ...
    // Aggregate stats for this user's products using the VIEW
    // vw_manager_dashboard has one row per product; aggregate here for totals
    $stmt = $pdo->prepare(
        "SELECT
            ...
            COUNT(DISTINCT number_of_transactions)               AS total_transactions
    ...
    // Today's revenue (direct query — view aggregates by product not by date)
...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
    :root { --sidebar-bg:#4a4a4a; --sidebar-width:240px; --main-bg:#f4f5f7; --card-bg:#e2e4e8; --nav-item:#6b6b6b; --nav-text:#ddd; --nav-active:#e8e8e8; }
...
<div class="main-content">
  ...
  <div class="cards-grid">
    <div class="stat-card"><div class="label">Total Transactions</div>
      <div class="value"><?= (int)($txRow['total_transactions'] ?? 0) ?></div></div>
    ...
    <div class="stat-card"><div class="label">Total Items Sold</div>
      <div class="value"><?= (int)getStat('total_units_sold') ?></div></div>
    <div class="stat-card"><div class="label">Today's Revenue</div>
      <div class="value">₱<?= number_format((float)($todayRow['todays_revenue'] ?? 0), 2) ?></div></div>
  </div>
